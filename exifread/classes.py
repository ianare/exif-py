...
class IfdAttrs:
    ...
    @property
    def ntag_length(self) -> int:
        return 8 if self.magic_number == 43 else 2
...
class TagAttrs:
    ...
    def __init__(self, magic_number: int) -> None:
        ...
        self.fieldtype_length: int = 2

    @property
    def nvalues_length(self) -> int:
        return 8 if self.magic_number == 43 else 4

    @property
    def inlining_threshold(self) -> int:
        """
        In classic TIFF, the tag data was
        written inside the tag structure,
...
class ExifHeader:
    ...
    def _process_tag(
        ...
        offset = (
            ...
            + self.tag_attrs.id_length
            + self.tag_attrs.fieldtype_length
            + self.tag_attrs.nvalues_length
        )

        # If the value fits in 4 bytes (8 bytes in the case of bigtiff), it is inlined, else we
        # need to jump ahead again.
        if count * type_length > self.tag_attrs.inlining_threshold:
    ...
    def decode_maker_note(self) -> None:
        ...
        if "NIKON" in make:
            if note.values[0:7] == [78, 105, 107, 111, 110, 0, 1]:
                ...
                self.dump_ifd(
                    note.field_offset, "MakerNote", tag_dict=makernote.nikon.TAGS_NEW
                )
            return
        ...
        if make.startswith("OLYMPUS"):
            self.dump_ifd(
                note.field_offset + 8, "MakerNote", tag_dict=makernote.olympus.TAGS
            )
            # TODO
        ...
        # Canon
        if make == "Canon":
            self.dump_ifd(note.field_offset, "MakerNote", tag_dict=makernote.canon.TAGS)

            for i in (
                ("MakerNote Tag 0x0001", makernote.canon.CAMERA_SETTINGS),
    ...
    def _canon_decode_camera_info(self, camera_info_tag):
        ...
        for offset, tag in camera_info_tags.items():
            ...
            self.tags["MakerNote " + tag_name] = IfdTag(
                str(tag_value), 0, 0, tag_value, 0, 0
            )
